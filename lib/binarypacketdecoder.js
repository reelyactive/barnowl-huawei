 ...
 */
function decodeTransmitterFrame(data, index, raddecs, receiverId, timestamp) {
  timestamp = timestamp || data.readUInt32BE(index + 6) * 1000;
...
function decode(data, origin, time, options) {
  ...
  let timestamp = (options.timestampOnArrival ? time : null);
